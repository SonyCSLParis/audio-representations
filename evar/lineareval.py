...
from evar.utils import append_to_csv, hash_text, load_yaml_config, seed_everything
...
def to_embeddings(emb_ar, data_loader, device, _id=None, fold=1, cache=False, verbose=2):
    ...
    for X, y in tqdm(data_loader,
                     desc=f'Getting {_id} {data_loader.dataset.split} embeddings...',
                     leave=False):
        with torch.no_grad():
...
def _one_linear_eval(X, y, X_val, y_val, X_test, y_test, hidden_sizes, epochs, early_stop_epochs, lr, classes,
                     standard_scaler, mixup, debug, verbose=2):
    ...
    clf = TorchMLPClassifier2(hidden_layer_sizes=hidden_sizes, max_iter=epochs, learning_rate_init=lr,
                              ...
                              verbose=verbose, standard_scaler=standard_scaler, mixup=mixup, debug=debug)
...
def lineareval_downstream(config_file, task, options='', seed=42, lr=None, hidden=(), standard_scaler=True, mixup=False,
                          epochs=None, early_stop_epochs=None, unit_sec=None, step='1pass', verbose: int = 2):
    ...
    for fold in range(1, n_folds + 1):
        ...
        score, df = _one_linear_eval(X, y, X_val, y_val, X_test, y_test, hidden_sizes=hidden, epochs=epochs,
                                     early_stop_epochs=early_stop_epochs, lr=lr, classes=classes, verbose=verbose,
